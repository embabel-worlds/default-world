...
User-created apps (vibe-coded) live in `data/apps/` instead, served at
`/apps/workspace/{name}`; this directory serves at `/apps/world/{name}`. The
legacy flat `/apps/{name}` form checks `data/apps/{name}` first, falling back
to this directory, so a user can
